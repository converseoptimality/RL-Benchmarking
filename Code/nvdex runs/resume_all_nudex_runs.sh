...
GAMMA=0.99
HORIZON=700
LOG_ROOT="runs_Nudex_1/runs_Nudex_1resume"
        ...
        for algo in "${ALGS[@]}"; do
          ...
          case "$algo" in
            ...
            A2C)
              ...
              NUM_ENVS=8
              DEV="cpu"
              EXTRA_FLAGS+=" --a2c_n_steps 20 --a2c_lr 3e-4 --a2c_ent_coef 1e-4"
              ;;
            SAC)
              TOTAL_STEPS=1000000
              NUM_ENVS=1
              EXTRA_FLAGS+=" --off_batch_size 256 --off_buffer_size 700000 --off_learning_starts 10000 --off_train_freq 1 --off_gradient_steps 1 --off_lr 3e-4 --off_tau 0.005 --sac_ent_coef auto"
              ;;
            TD3)
              TOTAL_STEPS=1000000
              NUM_ENVS=2
              EXTRA_FLAGS+=" --off_batch_size 256 --off_buffer_size 700000 --off_learning_starts 25000 --off_train_freq 1 --off_gradient_steps 1 --off_lr 3e-4 --off_tau 0.005 --td3_policy_delay 2 --td3_target_noise 0.2 --td3_target_clip 0.5"
              ;;
            DDPG)
              TOTAL_STEPS=1000000
              NUM_ENVS=2
              EXTRA_FLAGS+=" --off_batch_size 256 --off_buffer_size 700000 --off_learning_starts 25000 --off_train_freq 1 --off_gradient_steps 1 --off_lr 3e-4 --off_tau 0.005"
              ;;
          esac

          for diff in "${DIFFS[@]}"; do
            # ---- SKIP already-run configs (your list): PPO/A2C/SAC with seed=7 and K=1 ----
            if [[ "$seed" == "7" && "$K" == "1" && ( "$algo" == "PPO" || "$algo" == "A2C" || "$algo" == "SAC" ) ]]; then
              echo "[SKIP] $algo seed=$seed K=$K diff=$diff (already run)"
              continue
            fi
